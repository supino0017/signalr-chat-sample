...
WORKDIR /src
COPY ["SignalRApp.csproj", "."]
RUN dotnet restore "./SignalRApp.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./SignalRApp.csproj" -c $BUILD_CONFIGURATION -o /app/build

# このステージは、最終ステージにコピーするサービス プロジェクトを公開するために使用されます
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./SignalRApp.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
